# Rodrigo Aguiar | AI & Data Engineer

Senior Engineer (20+ yrs) with a strong foundation in Industrial R&D and Systems Architecture. Currently dedicated to building AI agents and MLOps pipelines to deliver scalable, production-ready solutions.

**Current Focus:** AI Agents · LLM Orchestration · MLOps
**Availability:** GMT-3 (Brazil) — Fully flexible for US time zone alignment

### Professional Impact
* **AI & Automation:** Architecting autonomous RAG agents & automation pipelines to replace manual workflows with scalable AI.
* **Systems Architecture:** 20+ years of experience leading complex deployments — from LIMS/Ansys (ISO 17025) to scalable cloud infrastructure.
* **Cost & Efficiency:** Identified and implemented technical solutions increasing operational efficiency by 80% and generating >$40k savings per project.

### AI Engineering Pipeline
*From raw data to production: local processing, agent orchestration, retrieval, containerization, and cloud deployment.*

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-8A2BE2?style=for-the-badge)
![OCI](https://img.shields.io/badge/OCI-F80000?style=for-the-badge&logo=oracle&logoColor=white)

*Agent orchestration also includes LangGraph (stateful workflows) and LangSmith (observability & debugging).*

### Production Deployment & CI/CD
*Proven track record shipping full systems to [real clients](https://raguiar.eng.br/en/index.html#projeto) — not just prototypes.*

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=for-the-badge&logo=nginx&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)

### LLM APIs & Model Providers
*Production integration with cloud-based and self-hosted LLM providers.*

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Google_Gemini-8E75B2?style=for-the-badge&logo=googlegemini&logoColor=white)
![OpenRouter](https://img.shields.io/badge/OpenRouter-6467F2?style=for-the-badge)
![Hugging Face](https://img.shields.io/badge/Hugging_Face-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black)
![Ollama](https://img.shields.io/badge/Ollama-000000?style=for-the-badge&logo=ollama&logoColor=white)

### Projects
* **[Central Market Agent](https://github.com/RAguiarEng/central_market_agent):** Autonomous RAG agent for corporate knowledge.
* **[GenAI Insurance Bot](https://github.com/RAguiarEng/GenAI_Seguros):** AI-powered customer service assistant.

### Connect
- 🧑‍💻 [LinkedIn](https://www.linkedin.com/in/rsouzaaguiar/?locale=en-US)
- 💼 [Professional history](https://raguiar.eng.br/en/index.html)
- 💬 [WhatsApp](https://wa.me/5547992704018?text=Hi%20Rodrigo.%20I%20checked%20your%20GitHub%20profile.)
- 📧 [E-mail](mailto:raguiar.eng@gmail.com?subject=GitHub%20contact)
